...
void main() {
  ...
  test('gravar offline salva local, entra como pendente e aparece no gráfico mensal', () async {
    ...
    final mensal = await ChuvaRepository.instance.graficoMensal(bd: bd, fazendaId: fazenda, ano: 2026);
    expect(mensal.length, 12); // sempre 12 meses, mesmo sem lançamento nos outros
  ...
  test('gravar de novo na mesma data sobrescreve (mesma regra do servidor: 1 registro por dia/fazenda)', () async {
    await ChuvaRepository.instance.gravar(
      bd: bd, fazendaId: fazenda, data: DateTime(2026, 3, 10), volume: 15,
    );
    final existenteAntes = await ChuvaRepository.instance.buscarVolumeExistente(
      bd: bd, fazendaId: fazenda, data: DateTime(2026, 3, 10),
    );
    expect(existenteAntes, 15);

    await ChuvaRepository.instance.gravar(
      bd: bd, fazendaId: fazenda, data: DateTime(2026, 3, 10), volume: 22,
    );
    final existenteDepois = await ChuvaRepository.instance.buscarVolumeExistente(
      bd: bd, fazendaId: fazenda, data: DateTime(2026, 3, 10),
    );
    expect(existenteDepois, 22);

    // continua 1 registro só, não duplicou
    final marco = await ChuvaRepository.instance.graficoMensal(bd: bd, fazendaId: fazenda, ano: 2026);
    expect(marco.firstWhere((m) => m['mes'] == 3)['dias'], 1);
  });

  test('sincronização do servidor NÃO sobrescreve lançamento local ainda pendente', () async {
    // Usuário lança 25mm offline num dia.
    await ChuvaRepository.instance.gravar(
      bd: bd, fazendaId: fazenda, data: DateTime(2026, 5, 5), volume: 25,
    );
    ...
    await ChuvaDao.instance.salvarLoteDoServidor(bd, [
      {"id": 999, "local": int.parse(fazenda), "data": "2026-05-05", "volume": 5.0},
    ]);

    final valor = await ChuvaRepository.instance.buscarVolumeExistente(
      bd: bd, fazendaId: fazenda, data: DateTime(2026, 5, 5),
    );
    expect(valor, 25); // o lançamento local pendente prevaleceu
    expect(await ChuvaDao.instance.contarPendentes(bd), 1); // continua pendente
  });

  test('sincronização do servidor atualiza normalmente o que NÃO está pendente', () async {
    await ChuvaDao.instance.salvarLoteDoServidor(bd, [
      {"id": 1, "local": int.parse(fazenda), "data": "2026-06-01", "volume": 12.0},
    ]);

    expect(await ChuvaDao.instance.contarPendentes(bd), 0);
    final valor = await ChuvaRepository.instance.buscarVolumeExistente(
      bd: bd, fazendaId: fazenda, data: DateTime(2026, 6, 1),
    );
    expect(valor, 12);
  });

  test('gráfico anual sempre traz 5 anos (ano-4..ano), com zero nos anos sem lançamento', () async {
    await ChuvaRepository.instance.gravar(
      bd: bd, fazendaId: fazenda, data: DateTime(2026, 2, 1), volume: 100,
    );
    await ChuvaRepository.instance.gravar(
      bd: bd, fazendaId: fazenda, data: DateTime(2024, 2, 1), volume: 50,
    );

    final anual = await ChuvaRepository.instance.graficoAnual(bd: bd, fazendaId: fazenda, anoFinal: 2026);
